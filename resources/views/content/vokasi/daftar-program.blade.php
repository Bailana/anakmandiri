...
@section('title', 'Daftar Program Vokasi')
		...
		<div class="card mb-4">
			<div class="card-body d-flex justify-content-between align-items-center">
					...
					document.addEventListener('DOMContentLoaded', function() {
						...
						document.querySelectorAll('.btn-edit-program').forEach(function(btn) {
							btn.addEventListener('click', function() {
								...
								form.action = '{{ url("vokasi/program-konsultan") }}' + '/' + id;
				...
				<div class="d-flex gap-2 flex-row">
					...
					<a href="{{ route('vokasi.index') }}" class="btn p-0 border-0 bg-transparent d-inline-flex d-sm-none align-items-center justify-content-center" style="width:44px;height:44px;border-radius:12px;min-width:44px;min-height:44px;">
						<i class="ri-arrow-left-circle-fill" style="font-size:2em;font-weight:bold;"></i>
					</a>
					<a href="{{ route('vokasi.index') }}" class="btn btn-secondary btn-sm d-none d-sm-inline-flex align-items-center" style="border-radius:12px;">
...
<div class="row">
	<div class="col-12">
		<form method="GET" action="{{ route('vokasi.daftar-program') }}" class="d-flex gap-2 align-items-end flex-wrap">
			...
			<a href="{{ route('vokasi.daftar-program') }}" class="btn btn-outline-secondary" title="Reset">
							...
							<td>
								<div class="d-flex gap-2 align-items-center">
									<div class="d-none d-sm-flex gap-2 align-items-center">
										...
										<form method="POST" action="{{ route('vokasi.program-konsultan.destroy', $p->id) }}" class="d-inline m-0 p-0 align-self-center">
											@csrf
											@method('DELETE')
									...
									<div class="d-inline-block d-sm-none dropdown">
										...
										<ul class="dropdown-menu dropdown-menu-end">
											...
											<li>
												<form method="POST" action="{{ route('vokasi.program-konsultan.destroy', $p->id) }}" class="d-inline">
													@csrf
													@method('DELETE')
								...
								<script>
									document.addEventListener('DOMContentLoaded', function() {
										...
										const nextKodeBase = "{{ url('vokasi/next-kode') }}";
										if (jenisEl && kodeInput) {
											const fetchNextFor = (val) => {
												const prefix = map[val] || '';
												kodeInput.value = prefix;
												if (!val) return;
												const url = nextKodeBase + '/' + encodeURIComponent(val);
												console.debug('[vokasi] fetching next-kode', url);
												fetch(url)
													.then(r => {
														console.debug('[vokasi] response status', r.status, r.ok, r.headers.get('content-type'));
														return r.json();
													})
													.then(json => {
														console.debug('[vokasi] next-kode json', json);
														if (json && json.success && json.next) {
															kodeInput.value = json.next;
														} else if (json && json.success && json.prefix && json.nextNumber) {
															const pad = json.pad || 3;
															kodeInput.value = json.prefix + String(json.nextNumber).padStart(pad, '0');
														} else {
															kodeInput.value = prefix;
														}
													}).catch((err) => {
														console.warn('[vokasi] next-kode fetch failed', err);
														kodeInput.value = prefix;
													});
											};

											jenisEl.addEventListener('change', function() {
												fetchNextFor(this.value || '');
											});
											// also listen to input events for some custom select widgets
											jenisEl.addEventListener('input', function() {
												fetchNextFor(this.value || '');
											});
											// fallback: if an <option> is clicked in the native dropdown
											document.addEventListener('click', function(ev) {
												const opt = ev.target && ev.target.tagName === 'OPTION' ? ev.target : null;
												if (opt && opt.parentElement === jenisEl) {
													fetchNextFor(opt.value || '');
												}
											});

											// If modal already has a selection (e.g. page prefilled), fetch on load
											if (jenisEl.value) {
												fetchNextFor(jenisEl.value);
											}

											// Also fetch whenever the modal is shown to ensure fresh next number
											const modal = document.getElementById('modalAddProgramMaster');
											if (modal) {
	...
	<div class="modal-dialog modal-dialog-scrollable">
		<form action="{{ route('vokasi.program-konsultan.store') }}" method="POST" class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="mb-3">
					<label class="form-label">Kode Program</label>
					<input type="text" name="kode_program" class="form-control" value="" placeholder="(akan terisi setelah memilih Jenis Vokasi)" readonly aria-readonly="true">
				</div>
				<div class="mb-3">
					<label class="form-label">Nama Program</label>
